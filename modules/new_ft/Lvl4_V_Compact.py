...
class FateAndTransport:
    ...
    def run(self, inputs={}):
        ...
        dsoilA1 = self.env['soilA1']
        dsoilA2 = self.env['soilA2']
        dsoilA3 = self.env['soilA3']

        sedFWA = self.env['freshwA']
        sedSWA = self.env['seawA']
        ...
        urban_slope = round(self.env['slope1'])
        natural_slope = round(self.env['slope2'])
        ag_slope = round(self.env['slope3'])
        ...
        RUSLE_urban = self.env['kffact1'] * urban_LS * self.env['C1'] * self.env['P1']
        ...
        RUSLE_natural = self.env['kffact2'] * natural_LS * self.env['C2'] * self.env['P2']
        ...
        RUSLE_agricultural = self.env['kffact3'] * ag_LS * self.env['C3'] * self.env['P3']
        ...
        soilpercSolid1 = 1 - self.env['soilWC1'] - self.env['soilAC1']
        soilpercSolid2 = 1 - self.env['soilWC2'] - self.env['soilAC2']
        soilpercSolid3 = 1 - self.env['soilWC3'] - self.env['soilAC3']
        dsoilpercSolid1 = 1 - self.env['deepsWC1'] - self.env['deepsAC1']
        dsoilpercSolid2 = 1 - self.env['deepsWC2'] - self.env['deepsAC2']
        dsoilpercSolid3 = 1 - self.env['deepsWC3'] - self.env['deepsAC3']
        ...
        SWpercSS = seassPerc
        SWpercWater = np.subtract(1, FWpercSS)

        aerVf = np.true_divide(aerV,airV)

        if 'freshssPerc' in self.env:
            FWpercSS = self.env['freshssPerc']

        if 'seassPerc' in self.env:
            SWpercSS = self.env['seassPerc']
        ...
        #AIR DIFFUSIVITY#
            ...
            airMD.append(airmd)
            awmtcair = 0.0292*((airSpeed[i]/365/24)**0.78)*(sqrt(waterA)**-0.11)*((visclist_air[i]/(self.env['airP']*(airMD[i]/10000)))**-0.69)
            for val in awmtcair:
                test = val
        ...
        #WATER DIFFUSIVITY#
            ...
            awmtcwater = 1e-6 + 1.44e-2*(fric_velocity*(fric_velocity**2.2)*(((visclist[i])/(self.env['waterP']*(waterMD[i]/10000))))**-0.5);
            awMTCwater.append(awmtcwater)

        #FW MTC#
            Rp = (1- self.env['fsedpercSolid'])**(-1/3)
            K23 = self.env['sedFWOC']*self.chem_prop['kOrgWater']
            Rc = 1 + ((self.env['dFWSedS']/1000)/(1-self.env['fsedpercSolid']))*K23
        ...
        #SW MTC#
            Rp = (1-self.env['ssedpercSolid'])**(-1/3);
            K23 = self.chem_prop['kOrgWater']*self.env['sedSWOC'];
            Rc = 1 + ((self.env['dSWSedS']/1000)/(1-self.env['ssedpercSolid']))*K23;
        ...
        #Mackay Handbook of Chemical Mass Transport in the Environment, pg 327#
            a = 0.0872142857
            b = 0.6886666667
            Scf = ((visclist[i]/1000)/(self.env['freshwP']*(waterMD[i]/10000)))
            Scs = ((visclist[i]/1000)/(self.env['seawP']*(waterMD[i]/10000)))
        #Manning roughness coefficient, pg 330 (0.403/21)#
            n = 0.0191904762
            Cd = 5*9.81*(n**2)
            FWvelocity = (self.climate['flow'][i])/self.env['freshwA'];
            #SWvelocity = (self.climate['flow'][i])/self.env['seawA'];
        ...
        #ZWATERSUB#
        zWaterSub = np.true_divide(zAirSub,self.chem_prop['kAirWater'])

        #ZOCTANOLSUB#
        zOctanolSub = np.multiply(self.chem_prop['kOctWater'],zWaterSub)


        #ZAEROSOLSUB#
        zAerosolSub = np.multiply(self.chem_prop['kAerAir'],zAirSub)

        #ZSUSPENDEDFRESHSUB#
        factor = np.true_divide(self.env['freshssP'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zSuspendedFreshSub = np.multiply(factor3,self.env['spOC'])

        #ZSUSPENDEDSEASUB#
        factor = np.true_divide(self.env['seassP'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zSuspendedSeaSub = np.multiply(factor3,self.env['spOC'])

        #ZSOILSUB1#
        factor = np.true_divide(self.env['dSS1'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        factor4 = np.true_divide(factor3,1000)
        zSoilSub1 = np.multiply(factor4,self.env['soilOC1'])
        ...
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zSoilSubSolid1 = np.multiply(factor3,self.env['soilOC1'])

        #ZSOILSUB2#
        factor = np.true_divide(self.env['dSS2'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        factor4 = np.true_divide(factor3,1000)
        zSoilSub2 = np.multiply(factor4,self.env['soilOC2'])
        ...
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zSoilSubSolid2 = np.multiply(factor3,self.env['soilOC2'])

        #ZSOILSUB3#
        factor = np.true_divide(self.env['dSS3'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        factor4 = np.true_divide(factor3,1000)
        zSoilSub3 = np.multiply(factor4,self.env['soilOC3'])
        ...
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zSoilSubSolid3 = np.multiply(factor3,self.env['soilOC3'])

        #ZSEDIMENTFRESHSUB#
        factor = np.true_divide(self.env['sedFWP'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        factor4 = np.true_divide(factor3,1000)
        zSedimentFreshSub = np.multiply(factor4,self.env['sedFWOC'])
        ...
        factor = np.true_divide(self.env['dFWSedS'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zSedimentFreshSubSolid = np.multiply(factor3,self.env['sedFWOC'])

        #ZSEDIMENTSEASUB#
        factor = np.true_divide(self.env['sedSWP'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        factor4 = np.true_divide(factor3,1000)
        zSedimentSeaSub = np.multiply(factor4,self.env['sedSWOC'])
        ...
        factor = np.true_divide(self.env['dSWSedS'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zSedimentSeaSubSolid = np.multiply(factor3,self.env['sedSWOC'])

        #ZDEEPSOILSUB1#
        factor = np.true_divide(self.env['dsoilP1'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zDeepSoilSub1 = np.multiply(factor3,self.env['dsoilOC1'])

        #ZDEEPSOILSUB2#
        factor = np.true_divide(self.env['dsoilP2'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zDeepSoilSub2 = np.multiply(factor3,self.env['dsoilOC2'])

        #ZDEEPSOILSUB3#
        factor = np.true_divide(self.env['dsoilP3'],1000)
        factor2 = np.multiply(zWaterSub,self.chem_prop['kOrgWater'])
        factor3 = np.multiply(factor2,factor)
        zDeepSoilSub3 = np.multiply(factor3,self.env['dsoilOC3'])

        #ZSOILPARTICLESUB1#
        factor = np.multiply(self.chem_prop['kOrgWater'],self.env['soilOC1'])
        factor2 = np.multiply(factor,self.env['dSS1'])
        factor3 = np.true_divide(factor2,1000)
        zSoilParticleSub1 = np.multiply(factor3,zWaterSub)

        #ZSOILPARTICLESUB2#
        factor = np.multiply(self.chem_prop['kOrgWater'],self.env['soilOC2'])
        factor2 = np.multiply(factor,self.env['dSS1'])
        factor3 = np.true_divide(factor2,1000)
        zSoilParticleSub2 = np.multiply(factor3,zWaterSub)

        #ZSOILPARTICLESUB3#
        factor = np.multiply(self.chem_prop['kOrgWater'],self.env['soilOC3'])
        factor2 = np.multiply(factor,self.env['dSS1'])
        factor3 = np.true_divide(factor2,1000)
        zSoilParticleSub3 = np.multiply(factor3,zWaterSub)

        #ZSEDPARTICLESUB#
        factor = np.multiply(self.chem_prop['kOrgWater'],self.env['sedFWOC'])
        factor2 = np.multiply(factor,self.env['dFWSedS'])
        factor3 = np.true_divide(factor2,1000)
        zSedParticleSub = np.multiply(factor3,zWaterSub)

        #ZSUSPARTICLESUB#
        factor = np.multiply(self.chem_prop['kOrgWater'],self.env['spOC'])
        factor2 = np.multiply(factor,self.env['freshssP'])
        factor3 = np.true_divide(factor2,1000)
        ...
        #ZSOIL1#
        factor = np.multiply(zWaterSub,self.env['soilWC1'])
        factor2 = np.multiply(zAirSub,self.env['soilAC1'])
        factor3 = np.multiply(zSoilSub1,soilpercSolid1)
        factor4 = np.add(factor,factor2)
        zSoil1 = np.add(factor3,factor4)

        #ZSOIL2#
        factor = np.multiply(zWaterSub,self.env['soilWC2'])
        factor2 = np.multiply(zAirSub,self.env['soilAC2'])
        factor3 = np.multiply(zSoilSub2,soilpercSolid2)
        factor4 = np.add(factor,factor2)
        zSoil2 = np.add(factor3,factor4)

        #ZSOIL3#
        factor = np.multiply(zWaterSub,self.env['soilWC3'])
        factor2 = np.multiply(zAirSub,self.env['soilAC3'])
        factor3 = np.multiply(zSoilSub3,soilpercSolid3)
        factor4 = np.add(factor,factor2)
        zSoil3 = np.add(factor3,factor4)

        #ZSEDIMENTFRESH#
        factor = np.subtract(1,self.env['fsedpercSolid'])
        factor2 = np.multiply(zWaterSub,factor)
        factor3 = np.multiply(zSedimentFreshSubSolid,self.env['fsedpercSolid'])
        zSedimentFresh = np.add(factor2,factor3)

        #ZSEDIMENTSEA#
        factor = np.subtract(1,self.env['ssedpercSolid'])
        factor2 = np.multiply(zWaterSub,factor)
        factor3 = np.multiply(zSedimentSeaSubSolid,self.env['ssedpercSolid'])
        zSedimentSea = np.add(factor2,factor3)

        #ZDEEPSOIL1#
        factor = np.multiply(zWaterSub,self.env['deepsWC1'])
        factor2 = np.multiply(zAirSub,self.env['deepsAC1'])
        factor3 = np.multiply(zDeepSoilSub1,dsoilpercSolid1)
        factor4 = np.add(factor,factor2)
        zDeepSoil1 = np.add(factor3,factor4)

        #ZDEEPSOIL2#
        factor = np.multiply(zWaterSub,self.env['deepsWC2'])
        factor2 = np.multiply(zAirSub,self.env['deepsAC2'])
        factor3 = np.multiply(zDeepSoilSub2,dsoilpercSolid2)
        factor4 = np.add(factor,factor2)
        zDeepSoil2 = np.add(factor3,factor4)

        #ZDEEPSOIL3#
        factor = np.multiply(zWaterSub,self.env['deepsWC3'])
        factor2 = np.multiply(zAirSub,self.env['deepsAC3'])
        ...
        Q_urban = np.multiply(factor,self.env['soilA1'])


        factor = np.true_divide(1000,self.env['CN_natural'])
        S_natural = factor - 10
        ...
        Q_natural = np.multiply(factor,self.env['soilA2'])


        factor = np.true_divide(1000,self.env['CN_ag'])
        ...
        Q_agricultural = np.multiply(factor,self.env['soilA3'])


        #####REACTION D VALUES#####

        #DAIRREACT#
        factor = np.multiply(airV,self.chem_prop['kDegAir'])
        dAirReact = np.multiply(factor,zAir)

        #DAEROSOLREACT#
        factor = np.multiply(aerV,self.chem_prop['kDegAero'])
        dAerosolReact = np.multiply(factor,zAerosolSub)

        #DWATERFRESHREACT#
        factor = np.multiply(freshwV,self.chem_prop['kDegWater'])
        dWaterFreshReact = np.multiply(factor,zWaterFresh)

        #DFRESHSSEDREACT#
        factor = np.multiply(fwSSedV,self.chem_prop['kDegSSed'])
        dFreshSSedReact = np.multiply(factor,zSuspendedFreshSub)

        #DWATERSEAREACT#
        factor = np.multiply(seawV,self.chem_prop['kDegWater'])
        dWaterSeaReact = np.multiply(factor,zWaterSea)

        #DSEASSEDREACT#
        factor = np.multiply(swSSedV,self.chem_prop['kDegSSed'])
        dSeaSSedReact = np.multiply(factor,zSuspendedSeaSub)

        #DSOILREACT1#
        factor = np.multiply(soilV1,self.chem_prop['kDegSoil'])
        dSoilReact1 = np.multiply(factor,zSoil1)

        #DSOILREACT2#
        factor = np.multiply(soilV2,self.chem_prop['kDegSoil'])
        dSoilReact2 = np.multiply(factor,zSoil2)

        #DSOILREACT3#
        factor = np.multiply(soilV3,self.chem_prop['kDegSoil'])
        dSoilReact3 = np.multiply(factor,zSoil3)

        #DSEDIMENTFRESHREACT#
        factor = np.multiply(sedFWV,self.chem_prop['kDegSed'])
        dSedimentFreshReact = np.multiply(factor,zSedimentFresh)

        #DSEDIMENTSEAREACT#
        factor = np.multiply(sedSWV,self.chem_prop['kDegSed'])
        dSedimentSeaReact = np.multiply(factor,zSedimentSea)

        #DDEEPSOILREACT1#
        factor = np.multiply(deepsV1,self.chem_prop['kDegSoil'])
        dDeepSoilReact1 = np.multiply(factor,zDeepSoil1)

        #DDEEPSOILREACT2#
        factor = np.multiply(deepsV2,self.chem_prop['kDegSoil'])
        dDeepSoilReact2 = np.multiply(factor,zDeepSoil2)

        #DDEEPSOILREACT3#
        factor = np.multiply(deepsV3,self.chem_prop['kDegSoil'])
        ...
        #DAFWDIFFUCSION#
        factor = np.multiply(awMTCair,self.env['freshwA'])
        factor2 = np.multiply(factor, zAirSub)
        factor3 = np.true_divide(1,factor2)
        factor4 = np.multiply(awMTCwater,self.env['freshwA'])
        ...
        #DASWWETDEP#
        factor = np.multiply(precipR,self.env['freshwA'])
        factor2 = np.multiply(aerVf,self.env['precipScavR'])
        factor3 = np.multiply(factor2,zAerosolSub)
        dAFWwetdep = np.multiply(factor,factor3)

        #DAFWDRYDEP#
        factor = np.multiply(self.env['aerDepR'],self.env['freshwA'])
        ...
        #DASWDIFFUSION#
        factor = np.multiply(awMTCair,self.env['seawA'])
        factor2 = np.multiply(factor,zAirSub)
        factor3 = np.true_divide(1,factor2)
        factor4 = np.multiply(awMTCwater,self.env['seawA'])
        factor5 = np.multiply(factor4,zWaterSub)
        factor6 = np.true_divide(1,factor5)
        factor7 = np.add(factor3,factor6)
        ...
        #DASWWETDEP#
        factor = np.multiply(precipR,self.env['seawA'])
        factor2 = np.multiply(aerVf,self.env['precipScavR'])
        factor3 = np.multiply(factor2,zAerosolSub)
        dASWwetdep = np.multiply(factor,factor3)

        #DASWDRYDEP#
        factor = np.multiply(self.env['aerDepR'],self.env['seawA'])
        factor2 = np.multiply(aerVf,zAerosolSub)
        ...
        #DASDIFFUSION1#
        factor = np.multiply(asMTC1,self.env['soilA1'])
        factor2 = np.multiply(factor,zAirSub)
        factor3 = np.true_divide(1,factor2)
        ...
        factor7 = np.multiply(self.env['soilA1'],factor6)
        factor8 = np.true_divide(self.env['soilpathlen'],factor7)

        factor9 = np.add(factor3,factor8)
        dASdiffusion1 = np.true_divide(1,factor9)

        #DASDIFFUSION2#
        factor = np.multiply(asMTC2,self.env['soilA2'])
        factor2 = np.multiply(factor,zAirSub)
        factor3 = np.true_divide(1,factor2)
        ...
        factor5 = np.multiply(waterMD,zWaterSub)
        factor6 = np.add(factor4,factor5)
        factor7 = np.multiply(self.env['soilA2'],factor6)
        factor8 = np.true_divide(self.env['soilpathlen'],factor7)
        ...
        #DASDIFFUSION3#
        factor = np.multiply(asMTC3,self.env['soilA3'])
        factor2 = np.multiply(factor,zAirSub)
        factor3 = np.true_divide(1,factor2)
        ...
        factor5 = np.multiply(waterMD,zWaterSub)
        factor6 = np.add(factor4,factor5)
        factor7 = np.multiply(self.env['soilA3'],factor6)
        factor8 = np.true_divide(self.env['soilpathlen'],factor7)
        ...
        #DASWETDEP1#
        factor = np.multiply(precipR,self.env['soilA1'])
        factor2 = np.multiply(aerVf,self.env['precipScavR'])
        factor3 = np.multiply(factor2,zAerosolSub)
        dASwetdep1 = np.multiply(factor,factor3)

        #DASWETDEP2#
        factor = np.multiply(precipR,self.env['soilA2'])
        factor2 = np.multiply(aerVf,self.env['precipScavR'])
        factor3 = np.multiply(factor2,zAerosolSub)
        dASwetdep2 = np.multiply(factor,factor3)

        #DASWETDEP3#
        factor = np.multiply(precipR,self.env['soilA3'])
        factor2 = np.multiply(aerVf,self.env['precipScavR'])
        factor3 = np.multiply(factor2,zAerosolSub)
        dASwetdep3 = np.multiply(factor,factor3)

        #DASDRYDEP1#
        factor = np.multiply(self.env['aerDepR'],self.env['soilA1'])
        factor2 = np.multiply(aerVf,zAerosolSub)
        dASdrydep1 = np.multiply(factor,factor2)

        #DASDRYDEP2#
        factor = np.multiply(self.env['aerDepR'],self.env['soilA2'])
        factor2 = np.multiply(aerVf,zAerosolSub)
        dASdrydep2 = np.multiply(factor,factor2)

        #DASDRYDEP3#
        factor = np.multiply(self.env['aerDepR'],self.env['soilA3'])
        factor2 = np.multiply(aerVf,zAerosolSub)
        ...
        #DSEDFWDIFFUSION# - MATLAB AND PYTHON ROUND THE VISCOSITIES OFF TO A DIFFERRENT NUMBER OF
                           ...
                           #AS WELL
        factor = np.multiply(self.env['freshwA'],sedfwMTC)
        dSedFWdiffusion = np.multiply(factor,zWaterSub)

        #DSEDFWDEP#
        factor = np.multiply(self.env['freshwA'],self.env['freshssDepR'])
        dSedFWdep = np.multiply(factor,zSusParticleSub)

        #DSEDFWSUS#
        factor = np.multiply(self.env['freshwA'],self.env['freshssSusR'])
        dSedFWsus = np.multiply(factor,zSedParticleSub)

        #DSEDFWBUR#
        factor = np.multiply(self.env['freshwA'],self.env['freshssBurR'])
        dSedFWbur = np.multiply(factor,zWaterSub)

        #####SEAWATER SEDIMENT INTERFACE#####

        #DSEDSWDIFFUSION#
        factor = np.multiply(self.env['seawA'],sedswMTC)
        dSedSWdiffusion = np.multiply(factor,zWaterSub)

        #DSEDSWDEP#
        factor = np.multiply(self.env['seawA'],self.env['seassDepR'])
        dSedSWdep = np.multiply(factor,zSusParticleSub)

        #DSEDSWSUS#
        factor = np.multiply(self.env['seawA'],self.env['seassSusR'])
        dSedSWsus = np.multiply(factor,zSedParticleSub)

        #DSEDSWBUR#
        factor = np.multiply(self.env['seawA'],self.env['seassBurR'])
        ...
        factor = np.multiply(dsoilA1,self.env['leachR'])
        dS1toDeepS1leach = np.multiply(factor,zWaterSub)

        factor = np.multiply(dsoilA2,self.env['leachR'])
        dS2toDeepS2leach = np.multiply(factor,zWaterSub)

        factor = np.multiply(dsoilA3,self.env['leachR'])
        dS3toDeepS3leach = np.multiply(factor,zWaterSub)

        ###FRESHWATER###

        #DDEEPSFWRUNS1#
        factor = np.multiply(dsoilA1,self.env['soilRunS'])
        dDeepSFWrunS1 = np.multiply(factor,zDeepSoilSub1)

        #DDEEPSFWRUNW1#
        factor = np.multiply(dsoilA1,self.env['soilRunW'])
        ...
        bg0 = np.array([self.bg['A'], self.bg['fW'], self.bg['sW'],
            self.bg['S1'], self.bg['S2'], self.bg['S3'], self.bg['fwSed'],
            self.bg['swSed'], self.bg['S1'], self.bg['S2'], self.bg['S3']])
        ...
        if len(fw_sed_fugacity) !=0:
            ...
            fsedpercWater = 1 - self.env['fsedpercSolid']
            sedFWVsolid = np.multiply(sedFWV, self.env['fsedpercSolid'])
        ...
        # Seawater #
            ...
            ssedpercWater = 1 - self.env['ssedpercSolid']
            bulk_seawater_sediment = (sw_sed_solid * self.env['ssedpercSolid']) + (sw_sed_water * ssedpercWater)
        ...
        if len(deep_soil1_fugacity) !=0:
            ...
            air_vol = np.multiply(soilV1, self.env['soilAC1'])
            water_vol = np.multiply(soilV1, self.env['soilWC1'])
        ...
        if len(deep_soil2_fugacity) !=0:
            ...
            air_vol = np.multiply(soilV2,self.env['soilAC2'])
            water_vol = np.multiply(soilV2,self.env['soilWC2'])
        ...
        if len(deep_soil3_fugacity) !=0:
            ...
            air_vol = np.multiply(soilV3, self.env['soilAC3'])
            water_vol = np.multiply(soilV3, self.env['soilWC3'])
...
if __name__ == "__main__":
    g = FateAndTransport()
    g.load_environment("SF_glycerin_beta")
    output = g.run()
